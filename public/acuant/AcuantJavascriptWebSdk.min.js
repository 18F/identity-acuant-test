var AcuantConfig = (function () {
  "use strict";
  return { acuantVersion: "11.9.0" };
})();
let config = {};
"undefined" != typeof acuantConfig &&
  0 !== Object.keys(acuantConfig).length &&
  acuantConfig.constructor === Object &&
  (config = acuantConfig),
  ...
  (AcuantJavascriptWebSdk = (function (e) {
    let t = {
      ...
      startInitializer: function (n, a = 0) {
        ...
        (L = 1 == a), T(i, n);
        let r = null;
        e && e.cdnPath && e.cdnPath.initializerUrl
          ? (r = e.cdnPath.initializerUrl)
          : e.path && (r = e.path),
          (y = new Worker(O(r, "AcuantInitializerWorker.min.js", a))),
          (y.onmessage = w),
          (function () {
            if (document.getElementById(t.BARCODE_READER_ID)) return;
      ...
      startImageWorker: function (e) {
        e
          ? S
            ? e()
            : (T(a, e), I(L ? 1 : 0))
          : M("startImageWorker did not have a user callback set");
      },
      startMetricsWorker: function (e) {
        e
          ? R
            ? e()
            : (T(r, e), b(L ? 1 : 0))
          : M("startMetricsWorker did not have a user callback set");
      },
      endImageWorker: function () {
        S.terminate(), (S.onmessage = null), (P = !1), (S = null);
      },
      endMetricsWorker: function () {
        R.terminate(), (R.onmessage = null), (C = !1), (R = null);
      },
      start: function (e, t = !1) {
        if (!e) return void M("start did not have a user callback set");
        const i = L ? 1 : 0;
        ...
        S && this.endImageWorker(), R && this.endMetricsWorker();
      },
      startWorkers: function (
        e,
        t = [this.ACUANT_IMAGE_WORKER, this.ACUANT_METRICS_WORKER],
        i = 0
      ) {
        e
          ? (T(n, e),
            t.includes(this.ACUANT_IMAGE_WORKER) && !S && I(i),
            t.includes(this.ACUANT_METRICS_WORKER) && !R && b(i))
          : M("startWorkers did not have a user callback set");
      },
      endWorkers: function (
        e = [this.ACUANT_IMAGE_WORKER, this.ACUANT_METRICS_WORKER]
      ) {
        e.includes(this.ACUANT_IMAGE_WORKER) &&
          S &&
          (S.terminate(), (S.onmessage = null), (P = !1), (S = null)),
      ...
      initialize: function (e, t, i, n = 0) {
        i
          ? (T(o, i),
            y
              ? B(y, "initialize", { creds: e, endpoint: t })
              : this.startInitializer(() => {
                  B(y, "initialize", { creds: e, endpoint: t });
                }, n))
          : M("initialize did not have a user callback set");
      },
      initializeWithToken: function (e, t, i, n = 0) {
        i
          ? (T(o, i),
            y
              ? B(y, "initializeWithToken", { token: e, endpoint: t })
              : this.startInitializer(() => {
                  B(y, "initializeWithToken", { token: e, endpoint: t });
                }, n))
          : M("initializeWithToken did not have a user callback set");
      },
      crop: function (e, t, i, n) {
        h
          ? n
            ? P && null != e
              ? (T(l, n),
                B(S, "crop", { imgData: e.data, width: t, height: i }))
              : n.onFail()
            : M("crop did not have a user callback set")
          : M("SDK was not initialized");
      },
      detect: function (e, t, i, n) {
        h
          ? n
            ? P && null != e
              ? (T(s, n),
                B(S, "detect", { imgData: e.data, width: t, height: i }))
              : n.onFail()
            : M("detect did not have a user callback set")
          : M("SDK was not initialized");
      },
      metrics: function (e, t, i, n) {
        h
          ? n
            ? C && null != e
              ? (T(m, n),
                B(R, "metrics", { imgData: e.data, width: t, height: i }))
              : n.onFail()
            : M("metrics did not have a user callback set")
          : M("SDK was not initialized");
      },
      moire: function (e, t, i, n) {
        h
          ? n
            ? C && null != e
              ? (T(c, n),
                B(R, "moire", { imgData: e.data, width: t, height: i }))
              : n.onFail()
            : M("moire did not have a user callback set")
          : M("SDK was not initialized");
      },
      sign: function (e, t) {
        h
          ? t
            ? P && e
              ? (T(p, t), B(S, "sign", { imgData: e }))
              : t.onFail()
            : M("sign did not have a user callback set")
          : M("SDK was not initialized");
      },
      verify: function (e, t) {
        h
          ? t
            ? P && e
              ? (T(u, t), B(S, "verify", { imgData: e }))
              : t.onFail()
            : M("verify did not have a user callback set")
          : M("SDK was not initialized");
      },
      ...
      addMetadata: function (
        e,
        {
          ...
          software: n = "Acuant JavascriptWeb SDK " +
            AcuantConfig.acuantVersion,
          imageDescription: a = null,
          dateTimeOriginal: r,
          userComment: o = "=".repeat(100),
        }
      ) {
        if (!h) return void M("SDK was not initialized");
        let l = {},
          s = {};
    ...
    function I(t = 0) {
      let i = null;
      e && e.cdnPath && e.cdnPath.imageUrl
        ? (i = e.cdnPath.imageUrl)
        : e.path && (i = e.path),
        A++,
    ...
    function b(t = 0) {
      let i = null;
      e && e.cdnPath && e.cdnPath.metricsUrl
        ? (i = e.cdnPath.metricsUrl)
        : e.path && (i = e.path),
        A++,
          ...
          if (n && "initialize" === n.func) {
            ...
            t.endInitializer(),
              i
                ...
                : M("initialize did not have a user callback set");
          } else M("initworker sent message without correct function tagging");
        else {
          g = !0;
          let e = k[i];
          e && e();
        }
      } else M("initworker sent message without anything in the body");
    }
    function E(e) {
      if (e) {
        let t = e.data;
        if (P)
          if (t && "detect" === t.func) {
            ...
            e
              ? t.type &&
                ...
                ? (function (e, t, i, n, a, r, o, l, s, m) {
                  ...
                  })(t.type, t.x1, t.y1, t.x2, t.y2, t.x3, t.y3, t.x4, t.y4, e)
                : e.onFail()
              : M("detect did not have a user callback set");
          } else if (t && "crop" === t.func) {
            const e = k[l];
            e
              ? t.imgData && t.width && t.height && t.type
                ? (function (e, t, i, n, a) {
                    a &&
                      (null != e && t >= 0 && i >= 0 && n >= 0
                        ? ((D = {
                            image: { data: e, width: t, height: i },
                            cardType: n,
                            dpi: F(t, i, 2 == n),
                          }),
                          a.onSuccess(D))
                        : a.onFail());
                  })(t.imgData, t.width, t.height, t.type, e)
                : t.error
                ? e.onFail(t.error)
                : e.onFail()
              : M("crop did not have a user callback set");
            ...
            e
              ...
              : M("getCvmlVersion did not have a user callback set");
          } else M("imageworker sent message without correct function tagging");
        else (P = !0), _();
      } else M("imageworker sent message without anything in the body");
    }
    function x(e) {
      if (e) {
        let t = e.data;
      ...
      } else M("metricsworker sent message without anything in the body");
    }
    function v(e, t, i) {
      return (
        t.x < i.x && t.y < i.y
    ...
    function O(e, t, i) {
      let n;
      return (
        null != e && e.length > 0 && 0 == i
          ? ((n = "/" === e.charAt(e.length - 1) ? e : e + "/"), (n += t))
          : (n = 0 != i ? e : t),
        n
      );
    }
    function B(e, t, i, n = !1) {
      let a = { func: t, data: i };
      n && i && i.imgData && i.imgData.buffer
        ? e.postMessage(a, [a.data.imgData.buffer])
        : e.postMessage(a);
    }
    function _() {
      const e = k[n],
...
!(function () {
  ...
  function a(e, a, r) {
    ...
    if ("Byte" == a)
      (o = e.length),
        o <= 4 ? (d = t(e) + p("\0", 4 - o)) : ((d = m(">L", [r])), (u = t(e)));
    else if ("Short" == a)
      (o = e.length),
        o <= 2
          ? (d = i(e) + p("\0\0", 2 - o))
  ...
  function r(e, t, i) {
    ...
    for (var c in e) {
      if (
        ...
        "0th" == t && [34665, 34853].indexOf(c) > -1)
      )
        continue;
      if ("Exif" == t && 40965 == c) continue;
      if ("1st" == t && [513, 514].indexOf(c) > -1) continue;
      let r = e[c],
        o = m(">H", [c]),
        p = f[t][c].type,
        u = m(">H", [d[p]]);
      ...
      (t = u(e)),
        (i = (function (e) {
          ...
          for (let i = 0; i < e.length; i++)
            if (
              ((t = e[i]),
              "؇" == t.slice(0, 2) && "Exif\0\0" == t.slice(4, 10))
            )
              return t;
          return null;
        })(t)),
  ...
  if (
    ...
    (e.insert = function (e, t) {
      ...
      let n = "؇" + m(">H", [e.length + 2]) + e,
        a = (function (e, t) {
          ...
          e.forEach(function (a, r) {
            "؇" == a.slice(0, 2) &&
              "Exif\0\0" == a.slice(4, 10) &&
              (i ? n.unshift(r) : ((e[r] = t), (i = !0)));
          }),
      ...
      return i && (a = "data:image/jpeg;base64," + l(a)), a;
    }),
    (e.load = function (e) {
      let t;
      if ("string" != typeof e)
      ...
      if (
        (delete i["0th"].first_ifd_pointer,
        34665 in i["0th"] &&
          ((a = i["0th"][34665]), (i.Exif = n.get_ifd(a, "Exif"))),
        34853 in i["0th"] &&
          ((a = i["0th"][34853]), (i.GPS = n.get_ifd(a, "GPS"))),
        40965 in i.Exif &&
          ((a = i.Exif[40965]), (i.Interop = n.get_ifd(a, "Interop"))),
        "\0\0\0\0" != r &&
          ((a = c(n.endian_mark + "L", r)[0]),
          (i["1st"] = n.get_ifd(a, "1st")),
          513 in i["1st"] && 514 in i["1st"]))
      ) {
    ...
    (e.dump = function (t) {
      ...
      let a,
        ...
        y = !1;
      (a = "0th" in i ? i["0th"] : {}),
        ("Exif" in i && Object.keys(i.Exif).length) ||
        ("Interop" in i && Object.keys(i.Interop).length)
          ? ((a[34665] = 1),
            (p = !0),
            (o = i.Exif),
            "Interop" in i && Object.keys(i.Interop).length
              ? ((o[40965] = 1), (h = !0), (l = i.Interop))
              : Object.keys(o).indexOf(
                  e.ExifIFD.InteroperabilityTag.toString()
                ) > -1 && delete o[40965])
          : Object.keys(a).indexOf(e.ImageIFD.ExifTag.toString()) > -1 &&
            delete a[34665],
        "GPS" in i && Object.keys(i.GPS).length
          ? ((a[e.ImageIFD.GPSTag] = 1), (f = !0), (s = i.GPS))
          : Object.keys(a).indexOf(e.ImageIFD.GPSTag.toString()) > -1 &&
            delete a[e.ImageIFD.GPSTag],
        "1st" in i &&
          "thumbnail" in i &&
          null != i.thumbnail &&
          ((y = !0), (i["1st"][513] = 1), (i["1st"][514] = 1), (c = i["1st"]));
      let g,
        S,
        P,
        R,
        ...
        E = 0,
        x = "";
      (p && ((g = r(o, "Exif", k)), (L = g[0].length + 12 * h + g[1].length)),
      f && ((S = r(s, "GPS", k + L)), (I = S.join("")), (b = I.length)),
      h) &&
        ((P = r(l, "Interop", k + L + b)), (w = P.join("")), (E = w.length));
      if (
        y &&
        ((R = r(c, "1st", k + L + b + E)),
        (C = (function (e) {
      ...
      let B = A[0] + v + G + T + A[1];
      return (
        p && (D = g[0] + F + g[1]), "Exif\0\0MM\0*\0\0\0\b" + B + D + I + w + x
      );
    }),
    (o.prototype = {
      get_ifd: function (e, t) {
        ...
        for (let t = 0; t < a; t++) {
          ...
          let a = c(this.endian_mark + "H", this.tiftag.slice(e, e + 2))[0],
            o = [
              c(this.endian_mark + "H", this.tiftag.slice(e + 2, e + 4))[0],
              c(this.endian_mark + "L", this.tiftag.slice(e + 4, e + 8))[0],
              this.tiftag.slice(e + 8, e + 12),
            ];
      ...
      convert_value: function (e) {
        ...
        if (1 == n)
          a > 4
            ? ((t = c(this.endian_mark + "L", r)[0]),
              (i = c(
                this.endian_mark + p("B", a),
                this.tiftag.slice(t, t + a)
              )))
            : (i = c(this.endian_mark + p("B", a), r.slice(0, a)));
        else if (2 == n)
          a > 4
            ? ((t = c(this.endian_mark + "L", r)[0]),
              (i = this.tiftag.slice(t, t + a - 1)))
            : (i = r.slice(0, a - 1));
        else if (3 == n)
          a > 2
            ? ((t = c(this.endian_mark + "L", r)[0]),
              (i = c(
                this.endian_mark + p("H", a),
                this.tiftag.slice(t, t + 2 * a)
              )))
            : (i = c(this.endian_mark + p("H", a), r.slice(0, 2 * a)));
        else if (4 == n)
          a > 1
            ? ((t = c(this.endian_mark + "L", r)[0]),
              (i = c(
                this.endian_mark + p("L", a),
                this.tiftag.slice(t, t + 4 * a)
              )))
            : (i = c(this.endian_mark + p("L", a), r));
        else if (5 == n)
          if (((t = c(this.endian_mark + "L", r)[0]), a > 1)) {
            ...
            i = [
              c(this.endian_mark + "L", this.tiftag.slice(t, t + 4))[0],
              c(this.endian_mark + "L", this.tiftag.slice(t + 4, t + 8))[0],
            ];
        else if (7 == n)
          a > 4
            ? ((t = c(this.endian_mark + "L", r)[0]),
              (i = this.tiftag.slice(t, t + a)))
            : (i = r.slice(0, a));
        else if (9 == n)
          a > 1
            ? ((t = c(this.endian_mark + "L", r)[0]),
    ...
    l = function (e) {
      ...
      for (; m < e.length; )
        (t = e.charCodeAt(m++)),
          ...
          (s = s + c.charAt(a) + c.charAt(r) + c.charAt(o) + c.charAt(l));
      return s;
    };
  ...
  if (void 0 === s)
    s = function (e) {
      ...
      for (e = e.replace(/[^A-Za-z0-9\+\/\=]/g, ""); m < e.length; )
        (a = c.indexOf(e.charAt(m++))),
          (r = c.indexOf(e.charAt(m++))),
          (o = c.indexOf(e.charAt(m++))),
          (l = c.indexOf(e.charAt(m++))),
          (t = (a << 2) | (r >> 4)),
          (i = ((15 & r) << 4) | (o >> 2)),
          (n = ((3 & o) << 6) | l),
  ...
  function c(e, t) {
    ...
    for (let t = 1; t < e.length; t++)
      if ("b" == e[t].toLowerCase()) n += 1;
      else if ("h" == e[t].toLowerCase()) n += 2;
      else {
    ...
    for (; (s = e[o]); ) {
      if ("b" == s.toLowerCase())
        ...
        (m = 2),
          ...
          (l = 256 * c.charCodeAt(0) + c.charCodeAt(1));
      else {
        if ("l" != s.toLowerCase()) throw new Error("'unpack' error. " + s);
        (m = 4),
          (c = t.slice(r, r + m)),
          i && (c = c.split("").reverse().join("")),
          (l =
            16777216 * c.charCodeAt(0) +
            65536 * c.charCodeAt(1) +
            256 * c.charCodeAt(2) +
            c.charCodeAt(3)),
  ...
  (f["0th"] = f.Image),
    ...
    (e.InteropIFD = { InteroperabilityIndex: 1 }),
    (e.GPSHelper = {
